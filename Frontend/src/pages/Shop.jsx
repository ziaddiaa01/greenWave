...
export async function loader() {
  ...
  return defer({ products: allProducts.products });
}
...
export default function Shop() {
  ...
  async function handleCartClick(productID) {
    ...
    if (!isLoggedIn()) {
      ...
      try {
        const response = await addItemToCart({ productId: productID, quantity: 1 });
  ...
  const renderProductsElements = useCallback(
    (products) => {
      ...
      const productsElements = displayedProducts.map((product) => {
        return (
          <div
            key={product._id}
            className="p-2 relative flex flex-col border rounded-md overflow-hidden bg-white m-2 shadow-xl transition duration-200 ease-in-out transform hover:shadow-green hover:scale-105"
          >
            <Link
              to={`${product._id}`}
              state={{
                ...
                {product.saleAmount ? (
                  <div className="flex gap-2 items-center">
                    ...
                    <h3 className="font-semibold my-1 text-gray-700 text-base">
                      {(product.price * (1 - product.saleAmount / 100)).toFixed(
                        2
                      )}{" "}
                      EGP
                    </h3>
            ...
            <FontAwesome
              onClick={() => handleCartClick(product._id)} // Pass a function reference
    ...
    [categoryFilter, priceFilter, ratingFilter, searchParams]
  );
